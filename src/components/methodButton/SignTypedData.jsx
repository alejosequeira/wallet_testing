...
const SignTypeData = ({ address }) => {
    ...
    return (
        <div className="formulario1">
            <button className="button" onClick={() => handleSignTypedData('v3')}>SIGN TYPED DATA V3</button>
            {signTypedData.v3 && (
                <div>
                    <AlertComponent
            ...
            <button className="button" onClick={() => handleSignTypedData('v4')}>SIGN TYPED DATA V4</button>
            {signTypedData.v4 && (
                <div>
                    <AlertComponent
            ...
            {showForm && (
                <>
                    <div className="formulario_one">
                        ...
                        <button onClick={() => downloadJson('erc20Params')} className="button_download">Download ERC20 Permit sample</button>
                        <button onClick={() => downloadJson('testParams')} className="button_download">Download OpenSea Contract sample</button>

                        <div style={{ "position": "relative" }}>
                            <button className="toggle_absolute_button" onClick={() => setCurrentJson(prev => prev === 'testParams' ? 'erc20Params' : 'testParams')}>
                                {currentJson === 'testParams' ? 'OpenSea' : 'ERC20'}
                            </button>
                            <textarea className="textarea_json" value={jsonFiles[currentJson]} onChange={(e) => setJsonFiles(prev => ({ ...prev, [currentJson]: e.target.value }))}></textarea>
                        </div>
                    </div></>)}
        </div>
    );
